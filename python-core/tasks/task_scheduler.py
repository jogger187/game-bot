...
class ScheduledTask:
    ...
    job_id: str | None = None # 工作單 ID
    max_runs: int = 0 # 0 代表無限
...
class TaskScheduler:
    ...
    def add_task(
        ...
        **kwargs,
        ...
        logger.info(
            ...
            f"(interval={interval}s, max_runs={max_runs})"
        )
        return scheduled

    def get_task_by_job_id(self, job_id: str) -> ScheduledTask | None:
        for st in self._scheduled_tasks:
            if st.job_id == job_id:
                return st
        return None

    def remove_task(self, task_name: str = None, job_id: str = None) -> bool:
        """移除排程任務"""
        for i, st in enumerate(self._scheduled_tasks):
            if (task_name and st.name == task_name) or (job_id and st.job_id == job_id):
                self._scheduled_tasks.pop(i)
                logger.info(f"🗑️ 排程已移除: {task_name or job_id}")
                return True
        return False
    ...
    def _tick(self) -> None:
        ...
        for st in self._scheduled_tasks:
            ...
            if st.max_runs > 0 and st.run_count >= st.max_runs:
                st.completed = True
                st.enabled = False
                logger.info(f"✅ 任務已達指定次數完成: {st.name} ({st.run_count}/{st.max_runs})")
                continue
    ...
    def status(self) -> list[dict]:
        ...
        return [
            ...
            for st in self._scheduled_tasks
            if st.job_id is not None  # 我們只回報有 job_id 的 Job
        ]
